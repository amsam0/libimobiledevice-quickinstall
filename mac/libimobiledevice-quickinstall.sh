...
hash brew 2> /dev/null || { echo >&2 "Please install Homebrew: https://brew.sh (if you already have it installed, make sure it's in your PATH!)"; exit 1; }
hash ldid 2> /dev/null || { echo "Installing ldid..."; brew install ldid; }
hash wget 2> /dev/null || { echo "Installing wget..."; brew install wget; }
...
for DOWNLOAD in "${DOWNLOADS[@]}"; do
  ...
  DOWNLOAD_URL="https://nightly.link/libimobiledevice/$DOWNLOAD/workflows/build/master/$DOWNLOAD-latest_macOS.zip"
